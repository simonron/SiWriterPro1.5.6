...
//Rare exceptions to indicate a bug in the _siwriterpro code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_siwriterpro = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_siwriterpro]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.siwriterpro.module"
